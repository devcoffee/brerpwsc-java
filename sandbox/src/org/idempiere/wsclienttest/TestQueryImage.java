...
public class TestQueryImage extends AbstractTestWS {
				...
				for (int i = 0; i < response.getDataSet().getRowsCount(); i++) {
					...
					for (int j = 0; j < response.getDataSet().getRow(i).getFieldsCount(); j++) {
						...
						if (field.getColumn().equals("BinaryData") && !field.getValue().toString().isEmpty()) {
							writeFile("img/QueryImageTest_" + response.getDataSet().getRow(i).getField("AD_Image_ID").getValue().toString() + ".png", Base64Util.decode(field.getValue().toString()));
						}
